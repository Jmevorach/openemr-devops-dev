...
#   - Image A: Local build from repository (default: ./docker/openemr/7.0.5/)
#   - Image B: Public Docker Hub image (default: openemr/openemr:7.0.5)
#
# Benchmarks measured:
            ...
            if [[ "${mysql_status}" = "running" ]]; then
                ...
                if [[ -n "${container_name}" ]]; then
                    ...
                    if [[ "${mysql_health}" = "healthy" ]]; then
                        ...
                        log_error "${display_name} is running but unhealthy"
                        log_info "${display_name} logs:"
                        docker compose -p "${PROJECT_NAME}" logs "${service_name}" 2>&1 | tail -20 || true
                        return 1
                    fi
                fi
                # If health status is "starting", continue waiting
